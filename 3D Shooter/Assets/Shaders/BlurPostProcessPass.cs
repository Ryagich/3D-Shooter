...
[System.Serializable]
public class BlurPostProcessPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        var materials = BlurPostProcessingMaterial.Instance;
        if (materials == null)
        {
        ...
        //---Custom effect here---
        var customEffect = stack.GetComponent<BlurPostProccess>();
        // Only process if the effect is active
        if (customEffect.IsActive())
        {
            var material = materials.customEffect;
            // P.s. optimize by caching the property ID somewhere else
            material.SetFloat("_BlurSize", customEffect.Strength.value);
