...
[PerRendererData] _MainTex("Sprite Texture", 2D) = "white" {}
		_BlurSize("Blur Size", float) = 0.05
		_CircleDistance("Circle", float) = 0.05
		_CircleDiff("Circle", float) = 0.05
		_Mult("Circle", Color) = (0,0,0,0)
	}
...
#include "UnityCG.cginc"
		...
		uniform half _CircleDistance;
		uniform half _CircleDiff;
		uniform half4 _Mult;
		...
		half4 fragBlur8(v2f_withBlurCoords8 i) : COLOR
		{
			...
			half m = length((uv - half2(0.5, 0.5)) / _MainTex_TexelSize);
			

			half4 oColor = tex2D(_MainTex, uv);
			half4 color = 0;
			for (int l = 0; l < 7; l++)
			{
			...
			half t = (m - _CircleDistance) / _CircleDiff;
			
			
			 t = min(max(t, 0), 1);
			
			 return lerp(oColor, color * _Mult , t);
		}
